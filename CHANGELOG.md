...
- ✨ **全局编辑预测(NES 完全体)**: 命令 `DS Autocomplete: 预测下一处编辑(全局)`(快捷键 **Ctrl+Alt+E**)——最近编辑历史+全文件发给模型, 预测文件**任意位置**的下一个修改; 光标自动跳过去, range 替换幽灵文, Tab 接受/Esc 拒绝
- 🗑 **删除类预测一键删除**: 模型建议删行时弹确认通知, 点删除整行消失(含换行不留空行), 忽略不动文件
- 🛡 输出协议用 old/new 文本定位不信行号(防幻觉); old 不在文件中逐字存在 → 拒绝展示
- ⌨ 默认快捷键避开 Code Runner 的 Ctrl+Alt+N(全局无 when 抢键实锤)
- 🐛 幂等供给: 跳转双触发(自动+显式)不再顶掉预计算幽灵文; 大文件(>60000字符)截断偏移重定基; 空响应状态栏复位
- 🐛 **debug 开关启动时初始化**(此前只在配置变化事件赋值, 启动前已开启的场景 dbg 永远不输出)
- 📝 nextEdit 全程无条件日志(输出面板 [nextEdit] 前缀), 排查"按了没反应"不用开 debug
- ♻️ chat 请求抽取共用 `requestChat`(comment-to-code 与编辑预测同管道); JSON mode(`response_format: json_object`)
- 🐛 mock SSE 修复: 补 chat 形态 delta.content(此前 chat 响应内容永远空串)
...
- ✨ **改正模式**(NES/Zeta 范式): 模型输出与光标前行"相似但不同"且该行在最近编辑缓冲 → 补全项 range 覆盖旧行, Tab 接受=替换而不是追加。安全门: 非最近编辑行绝不触发, 防误删
- 🐛 注释前缀对齐: 模型改正/重复前常先输出解释注释("# 应该是…"), 注释 vs 代码相似度≈0 会把对齐打飞 → 前导注释跳过对齐但保留在文本里
